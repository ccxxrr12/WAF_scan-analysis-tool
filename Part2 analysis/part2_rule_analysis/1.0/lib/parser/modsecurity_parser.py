...
class SecRule():
    ...
    def jsonify_rule(self):
        ...
        json_rule['variable'] = self.variable.value if self.variable else None
        json_rule['operator'] = self.operator.value if self.operator else None
        json_rule['action'] = self.action.action if self.action else None
...
class Action():
    ...
    def __init__(self, action=None):
        ...
        if action and (action[0] == '"' or action[0] == '\''):
...
class Parser():
    ...
    def parse(self):
        ...
        # Return the rule even if not fully parsed
        newrule.rule = self.rule_string
        return newrule
...
def chain_rules(secrules):
    ...
    while i < len(secrules):
        ...
        end_chain = None
        if secrules[beg_chain].action and 'chain' in secrules[beg_chain].action.action:
            # loop through all the rules to find the end of the chain
            for index in range(beg_chain, len(secrules)):
                if secrules[index].action and 'chain' in secrules[index].action.action and \
...
def parse_file(rulefile):
    ...
    with open(rulefile, 'r', encoding='utf-8') as f:
        ...
        for index, line in enumerate(f):
            ...
            elif first_char_index < len(line) and line[first_char_index] == '#':
                continue
...
if __name__ == "__main__":
    ...
    rules = parse_file(args.rulefile)
    
    # 为了命令行使用保持打印功能
    for index, secrule in enumerate(rules):
