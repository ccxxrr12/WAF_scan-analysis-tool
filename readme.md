...
1.  **被动检测**
    *   **原理**：许多WAF会在HTTP响应头、Cookie或错误页面中留下独特的“指纹”。这是一种非侵入式的检测方法。
    *   **实现**：
...
2.  **主动探测**
    *   **原理**：故意发送一些看似恶意或异常的HTTP请求，观察WAF的响应与正常Web服务器的差异。这是一种更可靠但更具侵入性的方法。
    *   **实现**：
...
1.  **语法解析器开发**
    ...
    *   **实现路径（由易到难）**：
        ...
        *   **路径B（标准方法）**：使用**解析器生成器**。这是最强大和准确的方法。
            ...
            *   使用工具如 **https://www.antlr.org/** 或 Python的 **https://github.com/dabeaz/ply** 生成解析器。这将为你生成一个可以遍历的**抽象语法树**。

2.  **语义分析与高级功能**
    *   **依赖关系检测**：规则可以通过 `chain` 动作链接。解析器需要能识别出这些链式规则，并将其视为一个逻辑整体。
    *   **冲突检查**：检查是否有两条规则ID相同，或是否存在逻辑上可能矛盾的规则（这需要更深的语义理解）。
...
1.  **数据集构建**
    *   **数据收集**：使用你开发的第一部分（主动探测引擎）和第二部分（规则解析器）。
    ...
    *   **特征工程（核心）**：不能将原始HTTP请求直接扔给模型。需要提取特征：
...
2.  **模型选择与训练**
    ...
    *   **深度学习模型**：
        *   **文本分类模型**：将HTTP请求视为一段文本，使用CNN或RNN（LSTM）进行分类。这是非常主流和有效的方法。
        *   **结构化数据模型**：如果使用手工提取的特征，可以使用全连接神经网络或梯度提升树（如XGBoost）。
    *   **训练**：将数据集分为训练集、验证集和测试集，使用验证集调整超参数，最终在测试集上评估准确率、召回率等指标。
